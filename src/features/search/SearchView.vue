...
<template>
  <div class="bg-bg text-fg min-h-screen px-6 py-8 md:px-12">
    <!-- Title -->
    ...
    <p aria-live="polite" aria-atomic="true" class="sr-only">
      <template v-if="!store.loading && store.hasQuery && store.shows.length">
        {{ t('search.resultCount', store.shows.length) }}
      </template>
    </p>
